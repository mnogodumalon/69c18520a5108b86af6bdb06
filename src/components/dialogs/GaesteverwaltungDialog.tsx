...
interface GaesteverwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Gaesteverwaltung['fields']) => Promise<void>;
  defaultValues?: Gaesteverwaltung['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function GaesteverwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: GaesteverwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<Gaesteverwaltung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'gaesteverwaltung');
      await onSubmit(clean as Gaesteverwaltung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Gaesteverwaltung['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Gästeverwaltung bearbeiten' : 'Gästeverwaltung hinzufügen';
