...
interface DienstleisterverwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Dienstleisterverwaltung['fields']) => Promise<void>;
  defaultValues?: Dienstleisterverwaltung['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function DienstleisterverwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: DienstleisterverwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<Dienstleisterverwaltung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'dienstleisterverwaltung');
      await onSubmit(clean as Dienstleisterverwaltung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "firmenname": string | null, // Firmenname\n  "ansprechpartner_vorname": string | null, // Ansprechpartner Vorname\n  "ansprechpartner_nachname": string | null, // Ansprechpartner Nachname\n  "dl_email": string | null, // E-Mail-Adresse\n  "dl_telefon": string | null, // Telefonnummer\n  "kategorie": LookupValue | null, // Kategorie (select one key: "technik_av" | "location" | "musik_entertainment" | "fotografie_video" | "dekoration" | "transport" | "sonstiges" | "catering") mapping: technik_av=Technik / AV, location=Location, musik_entertainment=Musik / Entertainment, fotografie_video=Fotografie / Video, dekoration=Dekoration, transport=Transport, sonstiges=Sonstiges, catering=Catering\n  "dl_notizen": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Dienstleisterverwaltung['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Dienstleisterverwaltung bearbeiten' : 'Dienstleisterverwaltung hinzufügen';
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="technik_av">Technik / AV</SelectItem>
                <SelectItem value="location">Location</SelectItem>
                <SelectItem value="musik_entertainment">Musik / Entertainment</SelectItem>
