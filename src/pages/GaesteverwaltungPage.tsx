...
import type { Gaesteverwaltung } from '@/types/app';
...
export default function GaesteverwaltungPage() {
  const [records, setRecords] = useState<Gaesteverwaltung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Gaesteverwaltung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Gaesteverwaltung | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Gaesteverwaltung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getGaesteverwaltung());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Gaesteverwaltung['fields']) {
    await LivingAppsService.createGaesteverwaltungEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Gaesteverwaltung['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateGaesteverwaltungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteGaesteverwaltungEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Gästeverwaltung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Gästeverwaltung suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={8} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Gästeverwaltung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
